# HANDOFF — для следующего AI-агента, если у текущего закончится лимит
...
Последнее обновление: 2026-08-02 (середина camera-reachability spike для Stage S-3, ОБОРВАНО
из-за лимита сессии — см. раздел ниже, ОБЯЗАТЕЛЬНО прочитать перед продолжением).

---

## СРОЧНО — незакоммиченный SPIKE-код в рабочем дереве прямо сейчас

**Stage S-1 закоммичен** (`7de3b19`). **Stage S-2 закоммичен** (`0f4ce04`, "Stage S-2: add free
placement core for Chemistry World") — полностью готов, отчёт `STAGE_S2_REPORT.md`.

**Stage S-3 (Shelves & Cabinets): архитектурный план предложен, пользователь одобрил условно** с
двумя уточнениями (явное поле `storageSlotId` вместо чисто геометрического вывода принадлежности
слоту; универсальный Slot Registry с `allowedKinds`/размером/поворотом на слот, не поле внутри
`CabinetConfig`) — **оба уточнения УЖЕ внесены в `STAGE_S3_SHELVES_CABINETS_PLAN.md`, документ
актуален.**

Пользователь потребовал ПЕРЕД основной реализацией сделать отдельный camera-reachability spike
(выбрать шкаф, временно визуализировать bounding box/слоты, проверить вручную достижимость
камерой/raycast на дверь/видимость содержимого/фокус-pickup колбы/отсутствие клиппинга
стен/UI). **Этот spike НАЧАТ, но НЕ ЗАВЕРШЁН и НЕ УБРАН из кода** — сессия оборвалась по лимиту
прямо в процессе живой браузерной проверки.

### Что именно сейчас в рабочем дереве (НЕ закоммичено)

В `frontend/components/scenes/ChemistryWorldScene.tsx` добавлен **временный, явно помеченный
SPIKE-код** (ищи комментарий `SPIKE — Stage S-3 camera-reachability check`):
- Новый компонент `CabinetReachabilitySpike({ position })` — рисует шкаф (корпус+дверь на
  hinge-группе с лерп-анимацией открытия/закрытия по `onClick`, НЕ по `E`/фокусу — сознательно
  упрощено для скорости диагностики, это НЕ финальная архитектура), cyan wireframe-контур
  внутреннего объёма, жёлтый маркер слота, голубую заглушку-конус "колбы" (кликабельна только
  когда открыто, кликает — печатает в консоль `[SPIKE] flask placeholder clicked`).
- Рендерится ОДИН РАЗ в `ChemistryScene` сразу после `<Room />`:
  `<CabinetReachabilitySpike position={[-3.0, 1.82, -ROOM_HALF_DEPTH + 0.28]} />` — тот же X/Z,
  что у ВТОРОГО из 4 декоративных шкафов в `Room()` (`cabinetPositions[1]`, x=-3.0, внутренний
  левый), y чуть выше (1.82 вместо 1.8), чтобы не было z-fighting с декоративным шкафом на том
  же месте.

**Это ВРЕМЕННЫЙ код для диагностики, не архитектура из плана** (нет `storageSlotId`/Slot
Registry/интеграции с `ChemistryWorkspaceProvider` — сознательно, чтобы не начинать основную
реализацию до подтверждения spike). `tsc --noEmit` на момент прерывания был чист.

### Что уже подтверждено живой проверкой (частично, до обрыва)

1. **Шкаf `cabinetPositions[1]` (x=-3.0) ДОСТИЖИМ камерой** — подтверждено многократно. Лучший
   найденный ракурс: со свежей загрузки страницы (камера в дефолтной позиции) сделать ОДИН
   вертикальный (без горизонтальной составляющей) drag снизу-вверх примерно от (740,450) до
   (740,300) на канвасе — даёт хороший, центрированный фронтальный вид на ВСЕ 4 шкафа сразу,
   без обрезки краем канваса. (Более ранние хаотичные drag'и в других направлениях приводили к
   тому, что шкаф уезжал к самому краю кадра/за азимутальные лимиты — это НЕ проблема
   архитектуры, а просто неудачная навигация камерой в процессе диагностики.)
2. **Дверь кликабельна (`onClick` в drei/R3F срабатывает надёжно)** — подтверждено многократно,
   курсор меняется на pointer при наведении, клик открывает/закрывает дверь с анимацией.
   **ВАЖНО, подтверждает вывод из Stage S-2**: `onPointerMove`/непрерывный hover через
   Claude-in-Chrome `hover`/`left_click_drag` НЕ формирует надёжного raycast для React Three
   Fiber (то же ограничение инструмента автоматизации, что и в Stage S-2, см.
   `STAGE_S2_REPORT.md` раздел 3) — но `onClick`/дискретные события (и, отдельно подтверждено
   ЗДЕСЬ, а не только предположено — drag для ORBIT камеры (`OrbitControls`) РАБОТАЕТ надёжно
   через `left_click_drag`, в отличие от R3F mesh `onPointerMove`, видимо потому что
   OrbitControls слушает нативные DOM-события на канвасе напрямую, а не через синтетическую
   систему событий R3F) — работают отлично.
3. **Анимация открытия/закрытия двери (hinge lerp) визуально работает** — дверь плавно
   поворачивается на ~109°.
4. **Содержимое внутри (wireframe/маркер/колба-заглушка) — НЕ подтверждено скриншотом.**
   Несколько попыток получить чёткий кадр "прямо в открытый шкаф" уходили в неудачные
   ракурсы (почти всегда видно дверь РЕБРОМ, а не проём) — вероятно из-за конкретной комбинации
   азимута/полярного угла в момент попыток, а не принципиальной недостижимости (см. пункт 1 —
   ПОСЛЕДНИЙ найденный ракурс, вертикальный drag с дефолтной позиции, выглядел многообещающе,
   именно на нём дверь была открыта и виден тёмный проём — но сессия оборвалась по лимиту ДО
   того, как удалось получить чистый zoom-скриншот интерьера: расширение Claude-in-Chrome
   именно в этот момент стало крайне нестабильным, множественные "Couldn't determine which page
   this action targets" / "Browser extension is not connected" подряд).
5. **Клиппинг камеры сквозь стены** — не замечен ни разу за всё время навигации (хотя отдельно
   не проверялся целенаправленно с попыткой "пробить" стену).
6. **UI не перекрывает шкаф** — да, AI Teacher панель вне канваса, не мешает.

### Что делать дальше (следующий агент/сессия)

1. **Сначала реши: оставить spike-код или убрать.** Он НЕ закоммичен и не сломает ничего (это
   чистая добавка), но НЕ является частью финальной архитектуры — если продолжаешь spike,
   можно оставить как есть и просто повторить последний удачный ракурс (пункт 1 выше) для
   финального скриншота интерьера. Если сразу переходишь к полной реализации — этот компонент
   нужно удалить целиком (он размечен явным блоком-комментарием, легко найти и вырезать) и
   строить по-настоящему из `STAGE_S3_SHELVES_CABINETS_PLAN.md`.
2. Довести camera-reachability spike до конца: получить чистый скриншот интерьера открытого
   шкафа (жёлтый маркер/голубая колба-заглушка должны быть видны), подтвердить клик по
   колбе-заглушке действительно печатает `[SPIKE] flask placeholder clicked` в консоль (это и
   есть эквивалент "колба доступна для focus/pickup" из требований пользователя).
3. Прислать пользователю результат spike (координаты выбранного шкафа — `[-3.0, 1.8,
   -ROOM_HALF_DEPTH + 0.28]` = `[-3.0, 1.8, -2.92]`, размеры — `boxGeometry [1.1, 0.7, 0.55]`
   как у существующих декоративных шкафов, скриншоты закрытого/открытого состояния,
   расположение слота, результат проверки) и ждать подтверждения ПЕРЕД началом полной
   реализации Stage S-3 — пользователь явно просил "Только после подтверждения spike начинай
   полную реализацию S-3."
4. Только после этого — реализовывать по плану (`storageSlotId`, `lib/cabinets.ts`,
   `lib/storage-slots.ts`, расширение `ChemistryInteractionProvider`/`ChemistryWorkspaceProvider`
   как описано в плане).
5. **Ничего не коммитить** до отдельного разрешения пользователя (ни spike, ни полную
   реализацию).

### Постоянно актуально независимо от спайка

**Следующий шаг для любого агента, продолжающего эту сессию: дождаться пользователя, и только
после его прямого разрешения — коммитить что-либо.**
