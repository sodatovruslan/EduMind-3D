 ...
 * Содержит полную диагностическую панель (F3) для отслеживания реальных границ стен сцены.
 */
export function SandboxModeOverlay({
  ...
  // Горячая клавиша [F2] для переключения режимов и [F3] для вкл/выкл оверлея диагностики
  useEffect(() => {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-20 flex flex-col justify-between p-4 select-none">
      {/* Верхняя панель управления режимом */}
      ...
      {/* Диагностическая панель реальных стен (Dev Diagnostics Overlay) */}
      {isSandbox && showDiag && (
      ...
      {isSandbox && (
        <div className="pointer-events-none self-start rounded-xl border border-slate-700/80 bg-slate-900/85 p-3 text-xs backdrop-blur space-y-1">
          <p className="font-semibold text-emerald-400">Управление Sandbox (S7-V2.8):</p>
          <ul className="space-y-0.5 text-slate-300 font-mono text-[11px]">
            ...
            <li>• <span className="text-cyan-300">[F2]</span> — Режим камеры · <span className="text-cyan-300">[F3]</span> — Диагностика стен</li>
          </ul>
        </div>
      )}
    </div>
